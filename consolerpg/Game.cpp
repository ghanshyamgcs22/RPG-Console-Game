...
void Game::initGame()
{
	...
	
 {
	 ...
	 cin.get();
	 system("CLS");
	 if (this->chartr[activeveChar].getExp() >= this->chartr[activeveChar].getExpNext())
	 {
	...
	// cout << "main Menue" << "\n" << "\n";
	 ...
	 cout << "Active character name : " << this->chartr[this->activeveChar].getName()<<" No. " << this->activeveChar + 1 << "/" << this->chartr.size() << "\n\n";
	 cout << "0: Quit" << "\n";
	 cout << "1: Travel" << "\n";
	 //cout << "2: Shop" << "\n";
	 cout << "3: Level Up" << "\n";
	 ...
	 case 4://Rest
		 ...
		 if(time>60)
		 {
			 chartr[activeveChar].restExp(time);
		 }
		 ...
		 break;
	 case 5://Cahreacter sheet
		 chartr[activeveChar].printStats();
		 cin.get();
		 break;
	 case 6://New Chracter
         cin.ignore();
 ...
 void Game::creatNewCharacter()
	 {
      string name=" ";
		...
		activeveChar = chartr.size() - 1;
		chartr[activeveChar].initialize(name);
	 }
	 ...
	 {
		 this->chartr[activeveChar].levelUp();
		 if (this->chartr[activeveChar].getstatPoints() > 0)
		 {
			...
			// cout << "\n" << "5 :  " << "\n";
			 ...
			 {
			 case 0://strength
				 this->chartr[activeveChar].addToStat(0, 1);

				 break;

			 case 1://vitality
				 this->chartr[activeveChar].addToStat(1, 1);

				 break;
	
			 case 2://dexterity
				 this->chartr[activeveChar].addToStat(2, 1);

				 break;

			 case 3://intelligence
				 this->chartr[activeveChar].addToStat(3, 1);
				 break;
	 ...
	 void Game::selectCharacter()
	 {
		 ...
		 cin.ignore();
		 cout << "\n";
		 this->activeveChar = this->choice-1;
		 cout << this->chartr[this->activeveChar].getName() << " is selected!\n" << "\n";
	 ...
	 void Game::Travel()
	 {
		 character& currentCharacter = this->chartr[activeveChar];
